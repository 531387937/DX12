...
void ShapesApp::BuildFrameResources()
{
    ...
    {
        mFrameResources.push_back(std::make_unique<FrameResource>(md3dDevice.Get(), 1,(UINT)mMaterials.size(),(UINT)mAllRitems.size()));
    }
}
...
void ShapesApp::BuildRootSignature()
{
    ...
    CD3DX12_ROOT_SIGNATURE_DESC rootDesc(3,slotParameters,0,nullptr,D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);

    ComPtr<ID3DBlob> errorBlob = nullptr;
    ComPtr<ID3DBlob> serializedBlob = nullptr;

    HRESULT hr = D3D12SerializeRootSignature(&rootDesc,D3D_ROOT_SIGNATURE_VERSION_1,serializedBlob.GetAddressOf(),errorBlob.GetAddressOf());
    if(errorBlob!=nullptr)
    {
        ::OutputDebugStringA((char*)errorBlob->GetBufferPointer());
    }
    ThrowIfFailed(hr);
    md3dDevice->CreateRootSignature(0,serializedBlob->GetBufferPointer(),serializedBlob->GetBufferSize(),IID_PPV_ARGS(mRootSignature.GetAddressOf()));
}
...
void ShapesApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*> ritems)
{
    ...
    for (size_t i = 0; i < ritems.size(); i++)
    {
        ...
        UINT cbvIndex1 = gNumFrameResources*(UINT)mOpaqueRitems.size()+ mCurrFrameResourceIndex * (UINT)mMaterials.size() + ri->Mat->MatCBIndex;
