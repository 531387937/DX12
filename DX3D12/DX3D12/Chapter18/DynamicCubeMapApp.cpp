...
bool DynamicCubeMapApp::Initialize()
{
    ...
    mDynamicCubeMap = std::make_unique<CubeRenderTarget>(md3dDevice.Get(),CubeMapSize,CubeMapSize,DXGI_FORMAT_R8G8B8A8_UNORM);
...
void DynamicCubeMapApp::Draw(const GameTimer& gt)
{
	...
	mCommandList->SetGraphicsRootDescriptorTable(3, mDynamicCubeMap->Srv());
...
void DynamicCubeMapApp::BuildRenderItems()
{
	...
	globeRitem->IndexCount = globeRitem->Geo->DrawArgs["box"].IndexCount;
...
void DynamicCubeMapApp::DrawSceneToCubeMap()
{
    ...
    //转换回GENERIC_READ状态，以便在着色器中读取纹理数据
    mCommandList->ResourceBarrier(1,&CD3DX12_RESOURCE_BARRIER::Transition(mDynamicCubeMap->Resource(),D3D12_RESOURCE_STATE_RENDER_TARGET,
