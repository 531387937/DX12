...
DefferedShading::DefferedShading(HINSTANCE hInstance):D3DApp(hInstance)
{
	mSceneBounds.Center = XMFLOAT3(0.0f, 0.0f, 0.0f);
	mSceneBounds.Radius = sqrtf(10.0f*10.0f + 15.0f*15.0f);
}

DefferedShading::~DefferedShading()
{
    if(md3dDevice!=nullptr)
        FlushCommandQueue();
}

bool DefferedShading::Initialize()
{
...
void DefferedShading::Update(const GameTimer& gt)
{
	OnKeyboardInput(gt);
...
void DefferedShading::Draw(const GameTimer& gt)
{
...
void DefferedShading::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void DefferedShading::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void DefferedShading::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void DefferedShading::OnKeyboardInput(const GameTimer& gt)
{
...
void DefferedShading::AnimateMaterials(const GameTimer& gt)
{
}

void DefferedShading::UpdateObjectCBs(const GameTimer& gt)
{
...
void DefferedShading::UpdateMaterialBuffer(const GameTimer& gt)
{
...
void DefferedShading::UpdateShadowTransform(const GameTimer& gt)
{
...
void DefferedShading::UpdateMainPassCB(const GameTimer& gt)
{
...
void DefferedShading::UpdateShadowPassCB(const GameTimer& gt)
{
...
void DefferedShading::LoadTextures()
{
...
void DefferedShading::BuildRootSignature()
{
...
void DefferedShading::BuildShadersAndInputLayout()
{
    ...
    mShaders["shadowAlphaTestedPS"] =
        d3dUtil::CompileShader(L"Shaders\\Shadows.hlsl", alphaTestDefines, "PS", "ps_5_1");
	...
	mShaders["rtVS"] = d3dUtil::CompileShader(L"Shaders\\DefferedRT.hlsl", nullptr, "VS", "vs_5_1");
	mShaders["rtPS"] = d3dUtil::CompileShader(L"Shaders\\DefferedRT.hlsl", nullptr, "PS", "ps_5_1");
	mInputLayout =
	{
...
void DefferedShading::BuildShapeGeometry()
{
...
void DefferedShading::BuildSkullGeometry()
{
...
void DefferedShading::BuildPSOs()
{
	...
	// PSO for RT
	//
	D3D12_GRAPHICS_PIPELINE_STATE_DESC rtPsoDesc = opaquePsoDesc;
	rtPsoDesc.VS =
	{
		reinterpret_cast<BYTE*>(mShaders["rtVS"]->GetBufferPointer()),
		mShaders["rtVS"]->GetBufferSize()
	};
	rtPsoDesc.PS =
	{
		reinterpret_cast<BYTE*>(mShaders["rtPS"]->GetBufferPointer()),
		mShaders["rtPS"]->GetBufferSize()
	};
    rtPsoDesc.DepthStencilState.DepthEnable = false;
	ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&rtPsoDesc, IID_PPV_ARGS(&mPSOs["RT"])));
    //
    // PSO for sky.
    //
...
void DefferedShading::BuildFrameResources()
{
...
void DefferedShading::BuildMaterials()
{
...
void DefferedShading::BuildRenderItems()
{
    ...
    mRitemLayer[(int)RenderLayer::RT].push_back(quadRitem.get());
...
void DefferedShading::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
...
void DefferedShading::DrawSceneToGBuffer()
{
...
void DefferedShading::DrawSceneToShadowMap()
{
    ...
    mCommandList->ClearDepthStencilView(mShadowMap->Dsv(), D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0,
        0, nullptr);
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 7> DefferedShading::GetStaticSamplers()
{
